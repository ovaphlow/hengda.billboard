  ...
  public void insert(ReportRequest req, StreamObserver<ReportReply> responseObserver) {
    ...
    try (Connection conn = DBUtil.getConn()) {
      ...
      try (PreparedStatement ps = conn.prepareStatement(sql)) {
        ...
        ps.setString(4, body.get("data_id").toString());
        ps.setString(5, body.get("data_uuid").toString());
        ps.setString(6, body.get("category").toString());
        ps.setString(7, body.get("content").toString());
        ps.setString(8, body.get("datime").toString());
        ps.execute();
